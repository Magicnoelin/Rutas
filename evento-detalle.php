...
if (!empty($slug)) {
    try {
        ...
        if (!$conn->connect_error) {
            ...
            if ($lang == 'es') {
                // ESPAÑOL: Buscar directamente en cultural_events
                $sql = "SELECT 
                        e.id,
                        e.name AS titulo,
                        e.slug,
                ...
                // IDIOMAS EXTRANJEROS (de, en, fr, zh): Buscar primero en traducciones
                // 1. Buscar la traducción por slug e idioma
                $sql_trad = "SELECT 
                            event_id,
                            name AS titulo_trad,
                            slug AS slug_trad,
                            short_description AS short_desc_trad,
                            description AS descripcion_trad,
                            meta_title AS meta_title_trad,
                            meta_description AS meta_desc_trad,
                            program AS programa_trad,
                            target_audience AS audiencia_trad,
                            accessibility AS accesibilidad_trad
                        FROM cultural_events_trads 
                        WHERE slug = ? AND language_code = ?";
                
                $stmt_trad = $conn->prepare($sql_trad);
                $stmt_trad->bind_param("ss", $slug, $lang);
                $stmt_trad->execute();
                $result_trad = $stmt_trad->get_result();
                $traduccion = $result_trad->fetch_assoc();
                $stmt_trad->close();
                
                if ($traduccion) {
                    // 2. Buscar el evento original por event_id
                    $sql = "SELECT 
                            e.id,
                            e.name AS titulo,
                            e.slug,
                    ...
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("i", $traduccion['event_id']);
                } else {
                    // No se encontró traducción, buscar como español
                    $sql = "SELECT 
                            e.id,
                            e.name AS titulo,
                            e.slug,
            ...
            // Combinar datos si hay traducción
            if ($traduccion && $evento) {
                // Sobrescribir campos traducibles con la traducción
...
// Cada idioma tiene su propia canonical: español usa slug original, otros idiomas usan slug traducido
$slug_canonical = $slug; // Por defecto el slug de la URL
if ($evento) {
    if ($lang == 'es') {
        $slug_canonical = $evento['slug']; // Español: slug original
    } else {
        // Otros idiomas: usar slug traducido si existe, sino slug original
        $slug_canonical = $traduccion['slug_trad'] ?? $evento['slug'];
    }
}
...
<body>
    ...
    <main class="event-container">
        <?php if ($evento): ?>
            ...
            // Mostrar galería si hay fotos - MOVIDA ARRIBA
            if (!empty($fotos)): ?>
            <section class="event-gallery" style="margin: 30px 0 40px 0;">
                <h2 style="color: var(--primary); margin-bottom: 20px; border-bottom: 2px solid var(--primary); padding-bottom: 10px;">Galería del Evento</h2>
    ...
    ?>
            ...
            <!-- Información adicional de traducciones si está disponible -->
            <?php 
            $programa = $traduccion['programa_trad'] ?? '';
            $audiencia = $traduccion['audiencia_trad'] ?? '';
            $accesibilidad = $traduccion['accesibilidad_trad'] ?? '';
            
            if (!empty($programa) || !empty($audiencia) || !empty($accesibilidad)): ?>
            <section class="additional-info" style="margin: 40px 0; padding: 25px; background: #f9f9f9; border-radius: 10px; border-left: 5px solid var(--primary);">
                <h3 style="color: var(--primary); margin-bottom: 20px; font-size: 1.3rem;">
                    <i class="fas fa-info-circle" style="margin-right: 10px;"></i> Información Adicional
                </h3>
                
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                    <?php if (!empty($programa)): ?>
                    <div class="additional-item">
                        <h4 style="color: var(--dark-color); margin-bottom: 10px; font-size: 1.1rem;">
                            <i class="fas fa-calendar-check" style="color: var(--primary); margin-right: 8px;"></i> Programa
                        </h4>
                        <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #eee;">
                            <?php echo nl2br(htmlspecialchars($programa)); ?>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="additional-item">
                        <h4 style="color: var(--dark-color); margin-bottom: 10px; font-size: 1.1rem;">
                            <i class="fas fa-users" style="color: var(--primary); margin-right: 8px;"></i> Público Objetivo
                        </h4>
                        <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #eee;">
                            <?php echo nl2br(htmlspecialchars($audiencia)); ?>
                        </div>
                    </div>
                    <?php endif; ?>
                    
                    <?php if (!empty($accesibilidad)): ?>
                    <div class="additional-item">
                        <h4 style="color: var(--dark-color); margin-bottom: 10px; font-size: 1.1rem;">
                            <i class="fas fa-wheelchair" style="color: var(--primary); margin-right: 8px;"></i> Accesibilidad
                        </h4>
                        <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #eee;">
                            <?php echo nl2br(htmlspecialchars($accesibilidad)); ?>
                        </div>
                    </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="meta-grid">
                ...
                <div class="info-item">
                    ...
                    <?php echo date('d/m/Y', strtotime($evento['start_date'])); ?>
                </div>
                
                <!-- Fecha de fin (si existe y es diferente) -->
                <?php if (!empty($evento['end_date']) && $evento['end_date'] != $evento['start_date']): ?>
                <div class="info-item">
                    <i class="fas fa-calendar-check"></i>
                    <strong>Fecha de fin:</strong><br>
                    <?php echo date('d/m/Y', strtotime($evento['end_date'])); ?>
                </div>
                <?php endif; ?>
                ...
                <div class="info-item">
                    ...
                    <?php echo htmlspecialchars($evento['venue_address']); ?>
                </div>
                <?php endif; ?>
                ...
                <div class="info-item">
                    ...
                    <?php echo ($evento['is_active'] == 1) ? 'Activo' : 'No activo'; ?>
                </div>
            </div>
