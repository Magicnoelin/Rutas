...
### Planes Disponibles:
1. **Alojamientos:**
   - **Gratuito:** 0€ (hasta 2 alojamientos, 15 plazas)
   - **Básico:** 10€/mes o 50€/año + IVA (hasta 4 alojamientos, 30 plazas)
   - **Premium:** 10€/mes por alojamiento o cada 15 plazas (cálculo dinámico)
...
```bash
# Ejecutar los scripts SQL de configuración
mysql -u usuario -p nombre_base_datos < api/configurar_membresias_produccion.sql
mysql -u usuario -p nombre_base_datos < api/completar_tablas_membresias.sql
```
...
### Base de Datos:
- `api/configurar_membresias_produccion.sql` → Estructura principal con planes actualizados
- `api/completar_tablas_membresias.sql` → Tablas adicionales y vistas
...
- `api/calcular_precio_dinamico.php` → Cálculo dinámico para plan premium
...
- **Nuevo:** Plan premium con cálculo dinámico
...
## 🧮 Cálculo Dinámico Premium

### Para plan premium de alojamientos:
- **10€/mes por alojamiento** O
- **10€/mes por cada 15 plazas**
- Se toma el **mayor** de los dos cálculos
- **Ejemplo:** 3 alojamientos con 40 plazas:
  - Por alojamientos: 3 × 10€ = 30€/mes
  - Por plazas: ceil(40/15) = 3 bloques × 10€ = 30€/mes
  - **Precio final:** 30€/mes + IVA

### Función de cálculo:
```php
// Usar la función calculateDynamicPremiumPrice()
$price = calculateDynamicPremiumPrice(3, 40);
echo $price['monthly']['price_with_vat']; // 36.30€ (30€ + 21% IVA)
```

## 🛡️ Validación de Límites

El sistema ya incluye validación automática:
- **Usuarios gratuitos:** No pueden crear >2 alojamientos ni >15 plazas
- **Usuarios básicos:** No pueden crear >4 alojamientos ni >30 plazas
- **Usuarios premium:** Cálculo dinámico sin límites fijos
...
-- Probar cálculo dinámico
SELECT calculate_dynamic_price(3, 40) as precio_mensual;
```
...
### Cálculo dinámico incorrecto:
1. Verificar función `calculateDynamicPremiumPrice()`
2. Comprobar datos de alojamientos y plazas
3. Revisar lógica de cálculo (mayor entre alojamientos y plazas)
...
4. Añadir calculadora de precios en frontend
...
4. Análisis predictivo de precios
...
**Versión:** 2.0.0 (con cálculo dinámico)  
...
> 💡 **NOTA:** El sistema está diseñado para ser flexible. Los precios y límites pueden ajustarse fácilmente en la tabla `membership_plans` sin necesidad de cambios en el código.