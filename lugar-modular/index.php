 ...
 * Versión 1.0 — SSR + Skeleton Screens + SEO máximo
 * URL: /lugar/{slug}
 */
...
// ─── REDIRECCIONES 301 (migradas desde redirect_manager.php) ─────────────────
$redirects = [
...
if ($lugar) {
    $page_title = $lugar['meta_title']
        ?: ($lugar['name'] . ' — ' . ($lugar['municipality'] ?? '') . ' | Rutas Rurales');
    $page_desc  = $lugar['meta_description']
        ?: (substr(strip_tags($lugar['description'] ?? ''), 0, 160) ?: 'Lugar de interés en ' . ($lugar['municipality'] ?? ''));
} else {
    $page_title = 'Lugar de interés | Rutas Rurales';
    $page_desc  = 'Descubre este lugar de interés en Rutas Rurales';
}
...
if ($lugar) {
    ...
    if (isset($lugar['entry_fee'])) {
        $tourist['isAccessibleForFree'] = ($lugar['entry_fee'] == 0 || empty($lugar['entry_fee']));
    }
    if (!empty($lugar['pet_friendly'])) {
        $tourist['amenityFeature'] = [['@type' => 'LocationFeatureSpecification', 'name' => 'Admite mascotas', 'value' => true]];
    }
    ...
    $jsonld = json_encode([
        ...
        '@graph'   => [$webpage, $breadcrumb, $tourist],
...
<head>
    ...
    <meta name="description" content="<?php echo esc($page_desc); ?>">
    ...
    <meta property="og:description" content="<?php echo esc($page_desc); ?>">
    <meta property="og:image"       content="<?php echo esc($foto_og); ?>">
    <meta property="og:image:width"  content="1200">
    <meta property="og:image:height" content="630">
    <meta property="og:image:alt"    content="<?php echo esc($lugar['name'] ?? 'Lugar de interés'); ?>">
    <meta property="og:url"         content="<?php echo $canonical; ?>">
    <meta property="og:site_name"   content="Rutas Rurales">
    <meta property="og:locale"      content="es_ES">
    <?php if ($lugar && !empty($lugar['latitude'])): ?>
    <meta property="place:location:latitude"  content="<?php echo esc($lugar['latitude']); ?>">
    <meta property="place:location:longitude" content="<?php echo esc($lugar['longitude']); ?>">
    <?php endif; ?>
    ...
    <meta name="twitter:description" content="<?php echo esc($page_desc); ?>">
    <meta name="twitter:image"       content="<?php echo esc($foto_og); ?>">
    ...
    <!-- Preload imagen hero — fetchpriority=high para LCP -->
    <?php if (!empty($fotos[0])): ?>
    <link rel="preload" as="image" href="<?php echo esc(fixUrl($fotos[0])); ?>" fetchpriority="high">
    <?php endif; ?>

    <!-- Fuentes: font-display:optional → no bloquea FCP, sin CLS -->
    <style>
        @font-face {
        ...
        @font-face {
            ...
            font-display: optional;
            src: local('Montserrat Bold'), url('/fonts/montserrat-v31-latin-700.woff2') format('woff2');
            ascent-override: 85%;
            descent-override: 22%;
            line-gap-override: 0%;
        }
    ...
    <!-- ══════════════════════════════════════════════════════
         CSS CRÍTICO INLINE — todo lo que se ve above-the-fold
         ══════════════════════════════════════════════════════ -->
    <style>
        /* ── Variables ── */
        :root {
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* La imagen hero usa <img> real (no background-image) para que el LCP sea descubrible por Google */
        .lug-hero-bg-img {
        ...
        .lug-hero-free {
            background: var(--accent); color: var(--primary-d);
        ...
        /* ── DESCRIPCIÓN ── */
        .desc-text { line-height: 1.85; color: var(--text); font-size: 0.97rem; }
        ...
        .info-item .info-value { font-size: 0.92rem; font-weight: 600; color: var(--text); }
        ...
        .btn-contact:hover { transform: translateY(-2px); box-shadow: var(--shadow-h); }
        .btn-phone    { background: var(--primary); color: #fff; }
        .btn-whatsapp { background: #25D366; color: #fff; }
        .btn-email    { background: var(--accent-warm); color: #1a1a1a; }
        .btn-website  { background: var(--accent); color: var(--primary-d); }
        .contact-addr {
            display: flex; align-items: flex-start; gap: 10px;
            color: var(--text-l); font-size: 0.88rem;
            padding: 12px; background: var(--bg); border-radius: 8px;
        }

        /* ── MAPA ── */
        #lug-map-container { border-radius: var(--r); overflow: hidden; box-shadow: var(--shadow); margin-bottom: 24px; }
        ...
        .map-placeholder {
            ...
            color: var(--primary); gap: 12px; cursor: pointer;
        }
        ...
        .nearby-card:hover { box-shadow: var(--shadow-h); transform: translateY(-3px); color: inherit; }
        ...
        .nearby-card-name   { font-size: 0.85rem; font-weight: 700; color: var(--text); margin-bottom: 4px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
        .nearby-card-meta   { font-size: 0.75rem; color: var(--text-l); margin-bottom: 4px; }
        .nearby-card-price  { font-size: 0.8rem; font-weight: 700; color: var(--primary); margin-top: 4px; }
        ...
        .nearby-show-more button { background: none; border: 1px solid var(--primary); color: var(--primary); padding: 8px 20px; border-radius: 20px; font-size: 0.85rem; cursor: pointer; font-weight: 600; transition: all 0.2s; }
        .nearby-show-more button:hover { background: var(--primary); color: #fff; }
        .nearby-empty { grid-column: 1/-1; text-align: center; padding: 30px; color: var(--text-l); font-size: 0.9rem; }
        ...
        .info-card-title {
            ...
            color: var(--text-l); margin-bottom: 14px;
        }
        ...
        .info-list li .li-icon { font-size: 1rem; flex-shrink: 0; margin-top: 1px; }
        ...
        .cta-card h3 { font-size: 1rem; font-weight: 700; color: #fff; margin-bottom: 8px; }
        .cta-card p  { font-size: 0.82rem; color: rgba(255,255,255,0.88); margin-bottom: 14px; line-height: 1.5; }
        .btn-cta-primary {
            display: flex; align-items: center; justify-content: center;
            background: #fff; color: #2F5233;
        ...
        /* ── TOAST ── */
        .toast { position: fixed; bottom: 24px; right: 24px; background: var(--primary); color: #fff; padding: 12px 20px; border-radius: 8px; font-size: 0.9rem; font-weight: 600; z-index: 9998; transform: translateY(100px); opacity: 0; transition: all 0.3s ease; box-shadow: 0 4px 20px rgba(0,0,0,0.2); }
        .toast.show { transform: translateY(0); opacity: 1; }

        /* ── FOOTER ── */
        .site-footer { background: #1a2e1a; color: rgba(255,255,255,0.8); padding: 32px 20px; text-align: center; font-size: 0.85rem; }
        ...
        /* ── ERROR ── */
        .error-container { text-align: center; padding: 120px 20px 60px; max-width: 500px; margin: 0 auto; }
        .error-icon { font-size: 4rem; margin-bottom: 20px; }
        .error-container h1 { font-size: 1.6rem; margin-bottom: 12px; color: var(--primary); }
        .error-container p  { color: var(--text-l); margin-bottom: 24px; }
        .btn-back { display: inline-flex; align-items: center; gap: 8px; background: var(--primary); color: #fff; padding: 12px 24px; border-radius: 8px; font-weight: 700; text-decoration: none; }
...
<!-- ══════════════════════════════════════════════════════
     HERO — Renderizado SSR, visible inmediatamente
     ══════════════════════════════════════════════════════ -->
<section class="lug-hero" id="lug-hero">
    <!-- IMG real (no background-image) → Google descubre el LCP desde HTML -->
    <img id="heroBg"
    ...
    <div class="lug-hero-content">
        ...
        <div class="lug-hero-meta">
            ...
            <span>📍 <?php
                $parts = array_filter([$lugar['municipality'] ?? '', $lugar['province'] ?? '']);
            ...
            <span class="lug-hero-free" style="background:var(--accent-warm);color:#1a1a1a;">💶 <?php echo esc($lugar['entry_fee']); ?>€</span>
            <?php endif; ?>
        </div>
    </div>
</section>
...
<div class="lug-layout">
    ...
    <main>
        ...
        <!-- ▸ GALERÍA ── SSR + JS mejora con lightbox -->
        <?php if (!empty($fotos)): ?>
        <div class="lug-card">
        ...
        <!-- ▸ DESCRIPCIÓN — SSR completo -->
        <div class="lug-card">
            <div class="lug-card-body">
                <h2 class="lug-card-title">📋 Descripción</h2>
                <?php if (!empty($lugar['description'])): ?>
                <?php
                $allowed = '<strong><b><em><i><u><p><br><ul><ol><li><h2><h3><h4><span>';
                $desc    = strip_tags($lugar['description'], $allowed);
                ...
                <?php endif; ?>
                <?php else: ?>
                <p style="color:var(--text-l);font-style:italic;">Descripción no disponible.</p>
                <?php endif; ?>
            </div>
        </div>

        <!-- ▸ INFORMACIÓN PRÁCTICA — SSR completo -->
        <?php
        $hayInfo = !empty($lugar['opening_hours']) || !empty($lugar['entry_fee']) || !empty($lugar['visit_duration'])
                || !empty($lugar['best_season'])   || !empty($lugar['accessibility']) || !empty($lugar['pet_friendly'])
                || !empty($lugar['suitable_for_children']);
        if ($hayInfo):
        ?>
        <div class="lug-card">
            <div class="lug-card-body">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value"><?php echo esc($lugar['opening_hours']); ?></div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            <?php echo ($lugar['entry_fee'] == 0 || empty($lugar['entry_fee'])) ? 'Gratuita' : esc($lugar['entry_fee']) . '€'; ?>
                            <?php if (!empty($lugar['entry_fee_details'])): ?>
                            <br><small style="color:var(--text-l);font-weight:400;"><?php echo esc($lugar['entry_fee_details']); ?></small>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-label">Duración visita</div>
                        <div class="info-value"><?php echo esc($lugar['visit_duration']); ?></div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="info-item">
                        <div class="info-icon">🌸</div>
                        <div class="info-label">Mejor época</div>
                        <div class="info-value"><?php echo esc($lugar['best_season']); ?></div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value"><?php echo esc($lugar['accessibility']); ?></div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">Admitidas</div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="info-item">
                        <div class="info-icon">👶</div>
                        <div class="info-label">Familias</div>
                        <div class="info-value">Apto para niños</div>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div style="margin-top:20px;">
                    <div style="font-size:0.82rem;font-weight:700;color:var(--text-l);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:10px;">Instalaciones</div>
            ...
            <div class="lug-card-body">
                ...
                <div class="contact-btns">
                    ...
                    <a href="https://wa.me/34<?php echo preg_replace('/[^0-9]/', '', $lugar['phone']); ?>"
                       target="_blank" rel="noopener" class="btn-contact btn-whatsapp">💬 WhatsApp</a>
                    <?php endif; ?>
        ...
        <!-- ▸ MAPA — Carga diferida con skeleton -->
        ...
        <div id="lug-map-container" class="lug-card">
            <div id="map-placeholder" class="map-placeholder" onclick="initMap()">
                ...
                <p style="font-size:0.9rem;color:var(--text-l);">
                    <?php echo esc(implode(', ', array_filter([$lugar['municipality'] ?? '', $lugar['province'] ?? '']))); ?>
                </p>
        ...
        <!-- ▸ ALOJAMIENTOS CERCANOS — Skeleton + JS diferido -->
        <div id="nearby-alojamientos-section" class="lug-card" style="display:none;">
            <div class="lug-card-body">
                <h2 class="lug-card-title">🏠 ¿Dónde dormir cerca?</h2>
                <p style="color:var(--text-l);font-size:0.88rem;margin-bottom:16px;">Alojamientos rurales a pocos kilómetros de <?php echo esc($lugar['name']); ?>. ¡No dejes tu reserva para última hora!</p>
        ...
        <!-- ▸ ACTIVIDADES CERCANAS — Skeleton + JS diferido -->
        ...
        <!-- ▸ EVENTOS CERCANOS — Skeleton + JS diferido -->
        <div id="nearby-eventos-section" class="lug-card" style="display:none;">
            <div class="lug-card-body">
                ...
                <div class="nearby-show-more" id="more-eventos">
                    <button onclick="showMoreNearby('eventos')">Ver más eventos</button>
                </div>
            </div>
        </div>

        <!-- ▸ LUGARES CERCANOS — Skeleton + JS diferido -->
    ...
    <aside class="lug-sidebar">
        ...
        <div class="info-card">
            ...
            <ul class="info-list">
                <?php if (!empty($lugar['category_name'])): ?>
                <li><span class="li-icon">🏷️</span><span><?php echo esc($lugar['category_name']); ?></span></li>
                <?php endif; ?>

                <?php if (!empty($lugar['municipality'])): ?>
                <li><span class="li-icon">📍</span><span><?php echo esc($lugar['municipality']); ?><?php if (!empty($lugar['province'])): ?>, <?php echo esc($lugar['province']); ?><?php endif; ?></span></li>
                <?php endif; ?>

                <?php if (isset($lugar['entry_fee'])): ?>
                <li>
                    <span class="li-icon">🎫</span>
                    <span><?php echo ($lugar['entry_fee'] == 0 || empty($lugar['entry_fee'])) ? '✅ Entrada gratuita' : '💶 ' . esc($lugar['entry_fee']) . '€'; ?></span>
                </li>
                <?php endif; ?>

                <?php if (!empty($lugar['opening_hours'])): ?>
                <li><span class="li-icon">🕐</span><span><?php echo esc($lugar['opening_hours']); ?></span></li>
                <?php endif; ?>

                <?php if (!empty($lugar['visit_duration'])): ?>
                <li><span class="li-icon">⏱️</span><span>Visita: <?php echo esc($lugar['visit_duration']); ?></span></li>
                <?php endif; ?>

                <?php if (!empty($lugar['best_season'])): ?>
                <li><span class="li-icon">🌸</span><span>Mejor época: <?php echo esc($lugar['best_season']); ?></span></li>
                <?php endif; ?>

                <?php if (!empty($lugar['pet_friendly'])): ?>
                <li><span class="li-icon">🐾</span><span>Admite mascotas</span></li>
                <?php endif; ?>

                <?php if (!empty($lugar['suitable_for_children'])): ?>
                <li><span class="li-icon">👶</span><span>Apto para niños</span></li>
                <?php endif; ?>

                <?php if (!empty($lugar['phone'])): ?>
                <li><span class="li-icon">📞</span><a href="tel:<?php echo esc($lugar['phone']); ?>"><?php echo esc($lugar['phone']); ?></a></li>
                <?php endif; ?>

                <?php if (!empty($lugar['website'])): ?>
                <li><span class="li-icon">🌐</span><a href="<?php echo esc($lugar['website']); ?>" target="_blank" rel="noopener">Web oficial</a></li>
                <?php endif; ?>
            </ul>
...
(function() {
    ...
    ['click','scroll','keydown','touchstart'].forEach(function(e){window.addEventListener(e,loadGTM,{once:true,passive:true});});
