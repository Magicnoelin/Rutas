...
if ($item['created_by'] != $userId) {
    echo "<div style='padding: 20px; background: #fff3cd; border: 2px solid #856404; margin: 20px; border-radius: 8px;'>";
    echo "<h3>⚠️ Verificación de Permisos</h3>";
    echo "<p><strong>Usuario actual (sesión):</strong> " . $userId . "</p>";
    echo "<p><strong>Propietario del alojamiento:</strong> " . $item['created_by'] . "</p>";
    echo "<p><strong>Alojamiento:</strong> " . htmlspecialchars($item['name']) . " (ID: " . $item['id'] . ")</p>";
    echo "<hr>";
    echo "<p style='color: #721c24;'><strong>No tienes permiso para editar este alojamiento.</strong></p>";
    echo "<p>Este alojamiento pertenece al usuario ID " . $item['created_by'] . " y tú eres el usuario ID " . $userId . "</p>";
    echo "</div>";
    die();
}
?>
