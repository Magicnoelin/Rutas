...
<section class="apoyar-section apoyo-section">
    <div class="apoyar-container">
        ...
        <div class="custom-amount-box">
            <h3><i class="fas fa-heart" style="color:#e74c3c;"></i> Choisissez votre montant</h3>
            <p style="color:var(--text-light);font-size:.9rem;margin-bottom:1rem;">Tout montant est le bienvenu (minimum 1€)</p>
            ...
            <button class="btn-pay" onclick="pagarImporteLibre()" style="max-width:300px;">
                <i class="fas fa-heart"></i> Contribuer avec mon montant
            </button>
        </div>
    </div>
</section>
...
<div class="loading-overlay" id="loadingOverlay">
    <div class="loading-box">
        ...
        <p style="color:#666;">Redirection vers le paiement sécurisé...</p>
...
<script>
    ...
    async function pagar(code){
        mostrarLoading(true);
        try{const r=await fetch('/api/create_onetime_payment.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({concept_code:code,success_url:window.location.origin+'/fr/apoyar.php?payment=success',cancel_url:window.location.origin+'/fr/apoyar.php?payment=canceled'})});
        const d=await r.json();if(d.success&&d.data.checkout_url){window.location.href=d.data.checkout_url;}else{mostrarLoading(false);alert('Erreur : '+(d.error||'Impossible de créer le paiement'));}}
        catch(e){mostrarLoading(false);alert('Erreur de connexion.');}
    }
    async function pagarImporteLibre(){
        const a=parseFloat(document.getElementById('customAmount').value);
        if(!a||a<1){alert('Veuillez entrer un montant d\'au moins 1€');return;}
        mostrarLoading(true);
        try{const r=await fetch('/api/create_onetime_payment.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({concept_code:'APOYO_5',custom_amount:a,success_url:window.location.origin+'/fr/apoyar.php?payment=success',cancel_url:window.location.origin+'/fr/apoyar.php?payment=canceled'})});
        const d=await r.json();if(d.success&&d.data.checkout_url){window.location.href=d.data.checkout_url;}else{mostrarLoading(false);alert('Erreur : '+(d.error||''));}}
        catch(e){mostrarLoading(false);alert('Erreur de connexion.');}
    }
    ...
    function mostrarResultado(type,concept){
        const msgs={'CAFE_1':'Merci pour le café ! Cela nous donne de l\'énergie. ☕','CAFE_2':'Deux cafés ! Toute l\'équipe vous remercie. ☕☕','APOYO_5':'Merci pour votre contribution de 5€ ! 🌱','APOYO_10':'Merci pour votre contribution de 10€ ! 🌳','APOYO_20':'Incroyable ! Votre contribution de 20€ fait la différence. 🏔️','APOYO_50':'Vous êtes un héros rural ! Merci pour votre générosité. 🦅'};
        document.getElementById('resultIcon').textContent=type==='success'?'🎉':'😔';
        document.getElementById('resultTitle').textContent=type==='success'?'Merci beaucoup !':'Paiement annulé';
        document.getElementById('resultMessage').textContent=type==='success'?(msgs[concept]||'Votre paiement a été traité. Merci de soutenir le tourisme rural !'):'Aucun débit n\'a été effectué. Vous pouvez réessayer quand vous voulez.';
