-- ============================================
-- SCRIPT SQL: COMPLETAR TRADUCCIONES DE EVENTOS CULTURALES
...
SELECT 
    ...
    ce.venue_name as location,
    ce.municipality as localidad,
    ce.province,
    ce.start_date,
    ce.end_date,
    GROUP_CONCAT(DISTINCT cet.language_code ORDER BY cet.language_code) as existing_translations,
    COUNT(DISTINCT cet.language_code) as translation_count
...
SET 
    ...
    cet.short_description = CASE 
        ...
        THEN CONCAT('Traditional festival in ', ce.location, ', ', ce.province, ' featuring local culture, music, and traditions.')
    ...
    cet.meta_description = CASE 
        ...
        THEN CONCAT('Experience the ', ce.name, ' in ', ce.location, ', ', ce.province, '. Traditional Spanish festival with cultural activities, local food, and authentic celebrations. Perfect for international tourists.')
...
SET 
    ...
    cet.short_description = CASE 
        WHEN cet.short_description = '' 
        THEN CONCAT('Fête traditionnelle à ', ce.location, ', ', ce.province, ' mettant en valeur la culture locale, la musique et les traditions.')
        ELSE cet.short_description 
    END,
    cet.meta_title = CASE 
        WHEN cet.meta_title = '' 
    ...
    cet.meta_description = CASE 
        WHEN cet.meta_description = '' 
        THEN CONCAT('Vivez le ', ce.name, ' à ', ce.location, ', ', ce.province, '. Fête traditionnelle espagnole avec activités culturelles, nourriture locale et célébrations authentiques. Parfait pour les touristes internationaux.')
        ELSE cet.meta_description 
    END,
    cet.target_audience = CASE 
        WHEN cet.target_audience = '' 
...
SET 
    ...
    cet.short_description = CASE 
        WHEN cet.short_description = '' 
        THEN CONCAT('Traditionelles Fest in ', ce.location, ', ', ce.province, ' mit lokaler Kultur, Musik und Traditionen.')
        ELSE cet.short_description 
    END,
    cet.meta_title = CASE 
        WHEN cet.meta_title = '' 
    ...
    cet.meta_description = CASE 
        WHEN cet.meta_description = '' 
        THEN CONCAT('Erleben Sie das ', ce.name, ' in ', ce.location, ', ', ce.province, '. Traditionelles spanisches Fest mit kulturellen Aktivitäten, lokaler Küche und authentischen Feiern. Perfekt für internationale Touristen.')
        ELSE cet.meta_description 
    END,
    cet.target_audience = CASE 
        WHEN cet.target_audience = '' 
...
SET 
    ...
    cet.short_description = CASE 
        WHEN cet.short_description = '' 
        THEN CONCAT('西班牙', ce.province, ce.location, '的传统节日，展示当地文化、音乐和传统。')
        ELSE cet.short_description 
    END,
    cet.meta_title = CASE 
        WHEN cet.meta_title = '' 
    ...
    cet.meta_description = CASE 
        WHEN cet.meta_description = '' 
        THEN CONCAT('体验西班牙', ce.province, ce.location, '的', ce.name, '。西班牙传统节日，包含文化活动、当地美食和地道庆祝。非常适合国际游客。')
        ELSE cet.meta_description 
    END,
    cet.target_audience = CASE 
        WHEN cet.target_audience = '' 
...
SELECT 
    ...
    SUM(CASE WHEN cet_en.event_id IS NOT NULL THEN 1 ELSE 0 END) as events_with_en,
    SUM(CASE WHEN cet_fr.event_id IS NOT NULL THEN 1 ELSE 0 END) as events_with_fr,
    SUM(CASE WHEN cet_de.event_id IS NOT NULL THEN 1 ELSE 0 END) as events_with_de,
    SUM(CASE WHEN cet_zh.event_id IS NOT NULL THEN 1 ELSE 0 END) as events_with_zh,
    SUM(CASE WHEN cet_en.event_id IS NOT NULL AND cet_fr.event_id IS NOT NULL AND cet_de.event_id IS NOT NULL AND cet_zh.event_id IS NOT NULL THEN 1 ELSE 0 END) as events_complete
FROM cultural_events ce
...
SELECT 
    ...
    ce.name as event_name,
    ce.start_date,
    CASE WHEN cet_en.event_id IS NOT NULL THEN '✓' ELSE '✗' END as en,
    CASE WHEN cet_fr.event_id IS NOT NULL THEN '✓' ELSE '✗' END as fr,
    CASE WHEN cet_de.event_id IS NOT NULL THEN '✓' ELSE '✗' END as de,
    CASE WHEN cet_zh.event_id IS NOT NULL THEN '✓' ELSE '✗' END as zh,
    CASE 
...
SELECT 
    ...
    cet.language_code,
    ce.name as event_name,
    CASE WHEN cet.slug = '' OR cet.slug IS NULL THEN 'slug,' ELSE '' END as missing_slug,
    CASE WHEN cet.short_description = '' OR cet.short_description IS NULL THEN 'short_desc,' ELSE '' END as missing_short_desc,
    CASE WHEN cet.description = '' OR cet.description IS NULL THEN 'desc,' ELSE '' END as missing_desc,
    CASE WHEN cet.meta_title = '' OR cet.meta_title IS NULL THEN 'meta_title,' ELSE '' END as missing_meta_title,
    CASE WHEN cet.meta_description = '' OR cet.meta_description IS NULL THEN 'meta_desc,' ELSE '' END as missing_meta_desc,
    CASE WHEN cet.target_audience = '' OR cet.target_audience IS NULL THEN 'target,' ELSE '' END as missing_target
FROM cultural_events_trads cet
JOIN cultural_events ce ON cet.event_id = ce.id
WHERE ce.is_active = 1 
    AND (ce.start_date >= '2026-04-01' OR ce.end_date >= '2026-04-01')
    AND (cet.slug = '' OR cet.short_description = '' OR cet.description = '' OR cet.meta_title = '' OR cet.meta_description = '' OR cet.target_audience = '')
