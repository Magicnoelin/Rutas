/**
 * Módulo de Compartir para la página principal de Rutas Rurales
 */
document.addEventListener('DOMContentLoaded', () => {
    const shareButton = document.getElementById('home-hero-share-btn');
    if (!shareButton) {
        return;
    }

    const shareTitle = 'Rutas Rurales: Tu guía de turismo rural en España';
    const shareText = '¡He encontrado esta web para planificar escapadas rurales! Alojamientos, rutas y lugares con encanto.';
    const shareUrl = 'https://rutasrurales.io/';
    ...
    function showShareModal() {
        // Evitar duplicados
        if (document.getElementById('share-modal-home')) {
            return;
        }

        const modalHTML = `
            <div class="share-modal" id="share-modal-home">
                <div class="share-modal__overlay"></div>
                <div class="share-modal__content">
                    <button class="share-modal__close" aria-label="Cerrar">&times;</button>
                    <h3 class="share-modal__title">Compartir Rutas Rurales</h3>
                    <p class="share-modal__subtitle">¡Ayuda a otros a descubrir el turismo rural!</p>
                    <div class="share-modal__buttons">
                        <a href="https://api.whatsapp.com/send?text=${encodeURIComponent(shareText + ' ' + shareUrl)}" target="_blank" rel="noopener noreferrer" class="share-modal__btn share-modal__btn--whatsapp">
                        ...
                        <a href="https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareUrl)}&via=rutasrurales_io" target="_blank" rel="noopener noreferrer" class="share-modal__btn share-modal__btn--twitter">
                    ...
                    <div class="share-modal__copy-link">
                        ...
                        <button id="home-copy-btn">Copiar</button>
                    </div>
                </div>
            </div>
        `;
        ...
        const modal = document.getElementById('share-modal-home');
        const closeButton = modal.querySelector('.share-modal__close');
        const overlay = modal.querySelector('.share-modal__overlay');
        const copyButton = modal.querySelector('#home-copy-btn');
        ...
        copyButton.addEventListener('click', () => {
            const input = modal.querySelector('.share-modal__copy-link input');
            input.select();
