...
// Consulta ordenada por place_id ascendente
$stmt = $pdo->query("SELECT * FROM places_of_interest_trads ORDER BY place_id DESC, language_code ASC");
$rows = $stmt->fetchAll();

// Obtener lista de lugares para mostrar nombres
$stmtLugares = $pdo->query("SELECT id, name FROM places_of_interest WHERE is_active = 1");
$lugaresMap = [];
while ($l = $stmtLugares->fetch()) {
    $lugaresMap[$l['id']] = $l['name'];
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Admin - Places of Interest Trads</title>
    <style>
        ...
        .lang-zh { background-color: #cc0000; } /* Rojo Chino */
...
<body>
    ...
    <h2>Control de Traducciones: places_of_interest_trads</h2>
    ...
    <table>
        ...
        <tbody>
            <?php foreach ($rows as $row): 
                ...
                if ($lCode == 'zh') $langClass = 'lang-zh';
                
                $placeName = isset($lugaresMap[$row['place_id']]) ? $lugaresMap[$row['place_id']] : 'Unknown';
            ?>
            <tr>
                <td><?= $row['id'] ?></td>
                <td><strong><?= $row['place_id'] ?></strong></td>
                ...
                <?php 
                $fields = ['name', 'slug', 'short_description', 'description', 'meta_title', 'meta_description'];
                ...
                <td>
                    <a href="lugares_trads_editar.php?id=<?= $row['id'] ?>" class="btn-edit">Editar</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
