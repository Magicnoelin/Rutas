...
<style>
    :root {
        --primary: #2f5233; --primary-light: #4a7c59; --accent: #ff8f00;
        --bg: #f8faf8; --white: #ffffff; --text: #333; --text-light: #666;
        --border: #e0e8e0; --shadow: 0 4px 20px rgba(47,82,51,0.10); --radius: 16px;
    }
    body { padding-top: 80px; }
    @media (max-width: 992px) { body { padding-top: 70px; } }

    .hero { background: linear-gradient(135deg, var(--primary) 0%, var(--primary-light) 60%, #5a9e6f 100%); color: white; text-align: center; padding: 4rem 1.5rem 3.5rem; position: relative; overflow: hidden; }
    ...
    .pay-card { background: var(--white); border-radius: var(--radius); padding: 2rem 1.5rem; text-align: center; box-shadow: var(--shadow); border: 2px solid transparent; transition: all 0.3s ease; cursor: pointer; position: relative; overflow: hidden; }
    ...
    .custom-amount-box { background: white; border-radius: var(--radius); padding: 2rem; box-shadow: var(--shadow); max-width: 500px; margin: 2rem auto 0; text-align: center; }
    ...
    .amount-input-wrap span { padding: 0.8rem 1.2rem; background: var(--bg); font-size: 1.2rem; font-weight: 700; color: var(--primary); border-right: 2px solid var(--border); }
    .amount-input-wrap input { flex: 1; padding: 0.8rem 1rem; border: none; font-size: 1.2rem; font-weight: 700; color: var(--primary); outline: none; background: white; }
...
<section class="apoyar-section apoyo-section">
    <div class="apoyar-container">
        ...
        <div class="custom-amount-box">
            <h3><i class="fas fa-heart" style="color:#e74c3c;"></i> Choose your amount</h3>
            <p style="color:var(--text-light);font-size:0.9rem;margin-bottom:1rem;">Any amount is welcome (minimum €1)</p>
            ...
            <button class="btn-pay" onclick="pagarImporteLibre()" style="max-width:300px;">
                <i class="fas fa-heart"></i> Contribute my amount
            </button>
        </div>
    </div>
</section>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (params.get('payment') === 'success') {
            mostrarResultado('success', params.get('concept'));
    ...
    async function pagar(conceptCode) {
        mostrarLoading(true);
        try {
            const r = await fetch('/api/create_onetime_payment.php', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ concept_code: conceptCode,
                    success_url: window.location.origin + '/en/apoyar.php?payment=success',
                    cancel_url:  window.location.origin + '/en/apoyar.php?payment=canceled' })
            });
    ...
    async function pagarImporteLibre() {
        ...
        if (!amount || amount < 1) { alert('Please enter an amount of at least €1'); return; }
        mostrarLoading(true);
        try {
            const r = await fetch('/api/create_onetime_payment.php', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ concept_code: 'APOYO_5', custom_amount: amount,
                    success_url: window.location.origin + '/en/apoyar.php?payment=success',
                    cancel_url:  window.location.origin + '/en/apoyar.php?payment=canceled' })
            });
            const d = await r.json();
    ...
    function mostrarResultado(type, concept) {
        const msgs = {
            'CAFE_1':   'Thanks for the coffee! It gives us energy to keep improving Rutas Rurales. ☕',
            'CAFE_2':   'Two coffees! The whole team thanks you. ☕☕',
            'APOYO_5':  'Thank you for your €5 contribution! Rural tourism appreciates it. 🌱',
            'APOYO_10': 'Thank you for your €10 contribution! You are a great support. 🌳',
            'APOYO_20': 'Incredible! Your €20 contribution makes a real difference. 🏔️',
            'APOYO_50': 'You are a rural hero! Thank you for your generosity. 🦅',
        };
        ...
        document.getElementById('resultMessage').textContent = type === 'success'
            ? (msgs[concept] || 'Your payment has been processed. Thank you for supporting rural tourism!')
