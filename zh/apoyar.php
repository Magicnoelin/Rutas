...
<style>
    ...
    .hero h1{font-size:clamp(1.6rem,4vw,2.5rem);font-weight:800;margin-bottom:1rem;position:relative;z-index:1;text-shadow:0 2px 8px rgba(0,0,0,.4)}
    .hero p{font-size:1.05rem;opacity:.95;max-width:600px;margin:0 auto 2rem;position:relative;z-index:1;text-shadow:0 1px 4px rgba(0,0,0,.3)}
    ...
    <div class="apoyar-container">
        ...
        <div class="cards-grid" style="max-width:600px;margin:0 auto;">
            <div class="pay-card" onclick="pagar('CAFE_1')">
                <span class="icon">☕</span><h3>一杯咖啡</h3>
                <p>最小的举动，最大的意义</p>
            ...
            <div class="pay-card popular" onclick="pagar('CAFE_2')">
                ...
                <span class="icon">☕☕</span><h3>两杯咖啡</h3>
                <p>为整个团队。双倍能量！</p>
...
<div class="loading-overlay" id="loadingOverlay">
    <div class="loading-box"><i class="fas fa-spinner fa-spin"></i><p style="color:#666;">正在跳转到安全支付页面...</p><small style="color:#aaa;">由 Stripe 提供安全支付 🔒</small></div>
</div>
...
<script>
    ...
    async function pagar(code){mostrarLoading(true);try{const r=await fetch('/api/create_onetime_payment.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({concept_code:code,success_url:window.location.origin+'/zh/apoyar.php?payment=success',cancel_url:window.location.origin+'/zh/apoyar.php?payment=canceled'})});const d=await r.json();if(d.success&&d.data.checkout_url){window.location.href=d.data.checkout_url;}else{mostrarLoading(false);alert('错误：'+(d.error||''));}}catch(e){mostrarLoading(false);alert('连接错误，请重试。');}}
    async function pagarImporteLibre(){const input=document.getElementById('customAmount');const a=parseFloat(input.value);if(!a||a<1){input.focus();input.style.outline='2px solid #e74c3c';setTimeout(()=>input.style.outline='',2000);alert('请输入至少 €1 的金额');return;}mostrarLoading(true);try{const r=await fetch('/api/create_onetime_payment.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({concept_code:'APOYO_5',custom_amount:a,success_url:window.location.origin+'/zh/apoyar.php?payment=success',cancel_url:window.location.origin+'/zh/apoyar.php?payment=canceled'})});const d=await r.json();if(d.success&&d.data.checkout_url){window.location.href=d.data.checkout_url;}else{mostrarLoading(false);alert('错误：'+(d.error||''));}}catch(e){mostrarLoading(false);alert('连接错误。');}}
    ...
    function mostrarResultado(type,concept){const msgs={'CAFE_1':'感谢您的咖啡！☕','CAFE_2':'两杯咖啡！整个团队感谢您。☕☕','APOYO_5':'感谢您的 €5 捐款！🌱','APOYO_10':'感谢您的 €10 捐款！🌳','APOYO_20':'太棒了！您的 €20 捐款真的很有意义。🏔️','APOYO_50':'您是乡村英雄！感谢您的慷慨。🦅'};document.getElementById('resultIcon').textContent=type==='success'?'🎉':'😔';document.getElementById('resultTitle').textContent=type==='success'?'非常感谢！':'支付已取消';document.getElementById('resultMessage').textContent=type==='success'?(msgs[concept]||'您的支付已处理。感谢支持乡村旅游！'):'未产生任何费用。您可以随时重试。';document.getElementById('paymentResult').classList.add('show');}
