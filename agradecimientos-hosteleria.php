...
<!-- End Google Tag Manager -->
    ...
    <link rel="canonical" href="https://rutasrurales.io/agradecimientos-hosteleria.php">
    ...
    <style>
        ...
        * {
        ...
        .form-intro {
            ...
            box-shadow: 0 4px 20px rgba(47,82,51,0.08);
        ...
        .form-card:hover {
            box-shadow: 0 6px 30px rgba(47,82,51,0.12);
        }
        ...
        .form-group input.required-field:valid {
        ...
        .btn-submit:hover {
            ...
            box-shadow: 0 10px 35px rgba(47,82,51,0.45);
        }
...
<div class="form-section">
    ...
    <form id="hosteleria-form" enctype="multipart/form-data" novalidate>
        ...
        <div class="form-card">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="email" id="email" name="email" placeholder="tu@email.com" required class="required-field">
                </div>
                <div class="form-group">
                    <label for="telefono">Teléfono <span class="req">*</span></label>
                    <input type="tel" id="telefono" name="telefono" placeholder="+34 600 000 000" required class="required-field">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="form-row three">
                <div class="form-group">
                    ...
                    <input type="text" id="provincia" name="provincia" placeholder="Ej: Soria" value="">
                </div>
                <div class="form-group">
            ...
            <div id="horarios-container">
                ...
                <div class="horario-dia">
                    ...
                    <div class="horario-inputs">
                        <input type="time" name="horario_<?= $key ?>_apertura" placeholder="09:00">
                        <span>–</span>
                        <input type="time" name="horario_<?= $key ?>_cierre" placeholder="23:00">
        ...
        <div class="form-card">
            ...
            <div class="caracteristicas-grid" id="caracteristicas-grid">
                <?php
                $caracteristicas = [
                    ...
                    ['key' => 'niños', 'icon' => 'fas fa-baby', 'label' => 'Menú infantil'],
        ...
        <div class="form-card">
            ...
            <p style="font-size:0.88rem; color:#888; margin-bottom: 1.2rem;">
                ...
                Sube fotos del local, los platos, la terraza o el ambiente. Formatos aceptados: JPG, PNG, WEBP (máx. 5 MB cada una).
            </p>

            <div class="photos-grid">
                <?php for ($f = 1; $f <= 4; $f++): ?>
                <div class="photo-upload-box" id="photo-box-<?= $f ?>" onclick="document.getElementById('foto<?= $f ?>').click()">
                    <input type="file" id="foto<?= $f ?>" name="foto<?= $f ?>" accept="image/jpeg,image/png,image/webp" data-index="<?= $f ?>" onchange="previewPhoto(this, <?= $f ?>)" onclick="event.stopPropagation()">
                    ...
                    <div class="photo-preview" id="preview-<?= $f ?>">
                        ...
                        <button type="button" class="remove-btn" onclick="removePhoto(<?= $f ?>, event)" title="Eliminar foto"><i class="fas fa-times"></i></button>
                    </div>
                </div>
                <?php endfor; ?>
            </div>
        ...
        <div class="form-card">
            ...
            <div class="form-group">
                ...
                <textarea id="comentarios" name="comentarios" placeholder="Cuéntanos lo que quieras: historia del local, premios, especiales de temporada, eventos que organizáis..." style="min-height: 120px;"></textarea>
            </div>
        </div>
...
document.querySelectorAll('.check-item input[type="checkbox"]').forEach(function(cb) {
    cb.addEventListener('change', function() {
        ...
        if (this.checked) {
            item.classList.add('checked-style');
        } else {
            item.classList.remove('checked-style');
        }
    });
});

// ===== CERRADO CHECKBOX desactiva horarios =====
document.querySelectorAll('input[name$="_cerrado"]').forEach(function(cb) {
    cb.addEventListener('change', function() {
        const dia = this.name.replace('horario_', '').replace('_cerrado', '');
        const apertura = document.querySelector('input[name="horario_' + dia + '_apertura"]');
        const cierre = document.querySelector('input[name="horario_' + dia + '_cierre"]');
        if (apertura) apertura.disabled = this.checked;
        if (cierre) cierre.disabled = this.checked;
        if (apertura) apertura.style.opacity = this.checked ? '0.4' : '1';
        if (cierre) cierre.style.opacity = this.checked ? '0.4' : '1';
    });
});
