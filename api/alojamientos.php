...
try {
    ...
    // Para tabla accommodations, usar límite mayor por defecto
    $defaultLimit = ($tableName === 'accommodations') ? 200 : 20;
    $limit = isset($_GET['limit']) ? min(200, max(1, intval($_GET['limit']))) : $defaultLimit;
    $offset = ($page - 1) * $limit;
    ...
    // Solo mostrar alojamientos publicados
    // Verificar columnas y aplicar filtros
    try {
        ...
        if ($tableName === 'accommodations') {
            ...
            if (in_array('is_active', $columns)) {
                ...
                $params[':is_active'] = 1;
                error_log("DEBUG: Aplicando filtro is_active=1");
            } else {
                error_log("DEBUG: Columna is_active NO encontrada en $tableName");
            }
        } else {
        ...
        error_log("DEBUG: Error verificando columnas: " . $e->getMessage());
    ...
    if (isset($_GET['provincia']) && !empty($_GET['provincia'])) {
        $provinciaCol = ($tableName === 'accommodations') ? 'province' : 'Provincia';
        $where[] = "$provinciaCol = :provincia";
        $params[':provincia'] = sanitizeInput($_GET['provincia']);
    }
    ...
    $alojamientosProcesados = array_map(function($alojamiento) use ($tableName) {
        ...
        if ($isAccommodations) {
            ...
            // Crear array de fotos con URLs completas - buscar en TODAS las carpetas
            $fotos = [];
            ...
            $alojamiento['registration_number'] = $alojamiento['registration_number'] ?? '';
