...
try {
    switch ($action) {
        ...
        case 'list_conversations':
            ...
            $whereClause = "c.user_1_id = ? OR c.{$user2Col} = ?";

            // Calcular other_user_id según estructura
            $otherUserExpr = "CASE WHEN c.user_1_id = ? THEN c.{$user2Col} ELSE c.user_1_id END";
            ...
            $sql = "
                SELECT
                    ...
                    (SELECT COUNT(*) FROM messages m WHERE m.conversation_id = c.id AND m.is_read = 0 AND m.sender_id != ? AND m.sender_id IS NOT NULL) AS unread_count
